mergeInto(LibraryManager.library, {
  ...
  login: function(type, node_idx, api_idx) {
    selectWallet(UTF8ToString(type),UTF8ToString(node_idx),UTF8ToString(api_idx));
  },
  logout: function() {
    doLogoutAction();
  },
  ...
  asset_blend: function(t_id,t_memo,t_type){
    var id_t = UTF8ToString(t_id);
    var memo_t = UTF8ToString(t_memo);
    blendProfession(id_t,memo_t,UTF8ToString(t_type));
  },
